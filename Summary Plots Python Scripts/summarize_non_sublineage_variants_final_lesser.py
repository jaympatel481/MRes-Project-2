...
for sample in tqdm(sample_dirs):
    ...
    for i, row in dpmm_df.iterrows():
        ...
        for varid in ns_list:
            ...
            if counter<3:
                print("varids: ", varid)
                print("sublineage set: ", list(sublineage_set)[:5])
            counter+=1

            if ((varid in vaf_dict) and (str(varid) not in list(sublineage_set))):
...
df_top = df[df['variant_id'].isin(variant_order)].copy()
df_top['variant_id'] = pd.Categorical(df_top['variant_id'], categories=variant_order, ordered=True)
...
axes[0].set_ylabel('')
axes[0].tick_params(axis='y', which='major', labelsize=0)  # hide y axis labels
